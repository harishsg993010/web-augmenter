...
class BackgroundService {
  ...
  private async handleInjectScriptViaAPI(message: any, sender: chrome.runtime.MessageSender): Promise<any> {
    try {
      ...
      // Strategy 1: Try chrome.userScripts API (Chrome 120+)
      ...
      if (this.userScriptsAvailable) {
        try {
          ...
          console.warn('Web Augmenter: userScripts API failed, trying fallback...', userScriptError);
          // Fall through to next strategy
        }
      }

      // Strategy 2: Try direct execution in MAIN world (clean approach)
      // This works on most sites without strict CSP
      const executionId = `__webAugmenter_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      try {
        const results = await chrome.scripting.executeScript({
          target: { tabId },
          world: 'MAIN' as chrome.scripting.ExecutionWorld,
          func: (code: string, execId: string) => {
            // Direct execution - cleanest approach
            try {
              // Use Function constructor to execute code in MAIN world
              const fn = new Function(code);
              fn();
              // Mark success on window object
              (window as any)[execId] = true;
              return { success: true };
            } catch (error) {
              // CSP error - will be caught but won't throw to extension
              console.warn('Web Augmenter: Direct execution blocked by CSP:', error);
              return { success: false, error: String(error) };
            }
          },
          args: [script, executionId]
        });

        // Check if execution succeeded
        const result = results?.[0]?.result;
        if (result?.success) {
          console.log('Web Augmenter: Script injected via direct MAIN world execution', { id });
          return {
            success: true,
            id,
            method: 'direct'
        ...
        console.warn('Web Augmenter: Direct MAIN world execution failed, trying CSP bypass...', directError);

        // Strategy 3: CSP Bypass Pattern
        // Create a script element to bypass CSP restrictions
        // This works even on sites with strict CSP policies
        await chrome.scripting.executeScript({
          ...
          func: (code: string) => {
            try {
              ...
              // This bypasses CSP by using the extension's privileges
              const scriptEl = document.createElement('script');
              scriptEl.textContent = `(function() { try { ${code} } catch(e) { console.error('Web Augmenter script error:', e); } })();`;
              (document.head || document.documentElement).appendChild(scriptEl);
              scriptEl.remove();
            } catch (error) {
              console.error('Web Augmenter CSP bypass injection error:', error);
              throw error;
            }
          },
          args: [script]
        ...
        console.log('Web Augmenter: Script injected via CSP bypass pattern', { id });
        return {
          success: true,
          id,
          method: 'csp-bypass'
      ...
      console.error('Web Augmenter: All injection methods failed:', error);
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Unknown error'
      };
