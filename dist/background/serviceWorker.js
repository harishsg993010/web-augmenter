...
var WEB_FEATURE_BUILDER_SYSTEM_PROMPT = `
...
var UI_GENERATOR_SYSTEM_PROMPT = `
...
- **Closeable**: Include a close/minimize button (\xD7) in the top-right
...
async function fetchWeather(city) {
  try {
    ...
    tempDisplay.textContent = \`\${Math.round(data.main.temp)}\xB0C\`;
...
function renderTodos() {
  listContainer.innerHTML = todos.map(todo => \`
    <div class="todo-item \${todo.completed ? 'completed' : ''}">
      ...
      <span>\${todo.text}</span>
      <button onclick="deleteTodo(\${todo.id})">\xD7</button>
    </div>
  \`).join('');
}
\`\`\`
...
var LLMClient = class {
  ...
  async generateUIAtLocation(userInstruction, location, pageContext, screenshotBase64) {
    try {
      const messages = this.buildUIGenerationMessages(userInstruction, location, pageContext, screenshotBase64);
      const response = await this.makeAnthropicAPICall(messages, void 0, UI_GENERATOR_SYSTEM_PROMPT);
      return this.parseResponse(response);
    } catch (error) {
...
Please respond with valid JSON following the exact schema specified in the system prompt.`;
  ...
  buildUIGenerationMessages(userInstruction, location, pageContext, screenshotBase64) {
    const content = [];
...
Please respond with valid JSON following the exact schema specified in the system prompt.`;
  ...
  async makeAnthropicAPICall(messages, tabId, systemPrompt = WEB_FEATURE_BUILDER_SYSTEM_PROMPT) {
    if (!this.anthropic) {
      ...
      return {
        ...
        script: `(function() {
          ...
          toggle.addEventListener('click', () => {
            ...
            toggle.innerHTML = nowDark ? '\u2600\uFE0F' : '\u{1F319}';
          });
  ...
  parseResponse(response) {
    try {
      ...
      return {
        ...
        plan: parsed.plan.map((step) => String(step)),
...
var ScreenshotCapture = class {
  ...
  async captureWithRetry(options = {}, maxRetries = 2, delay = 100) {
    let lastError = null;
  ...
  isScreenshotSupported() {
    return typeof chrome !== "undefined" && typeof chrome.tabs !== "undefined" && typeof chrome.tabs.captureVisibleTab === "function";
  }
  async checkPermissions() {
    try {
      const permissions = await chrome.permissions.getAll();
      return permissions.permissions?.includes("activeTab") || permissions.permissions?.includes("tabs") || false;
    } catch (error) {
      console.warn("Could not check permissions:", error);
      return false;
    }
  }
  getScreenshotInfo(dataUrl) {
  ...
  compressScreenshot(dataUrl, quality = 80) {
    return new Promise((resolve, reject) => {
      const img = new Image();
      img.onload = () => {
...
var BackgroundService = class {
  ...
  async checkUserScriptsAvailability() {
    try {
    ...
    try {
      switch (message.type) {
        case MESSAGE_TYPES.PAGE_CONTEXT_READY:
          return await this.handlePageContext(message, sender);
        case "GET_SCREENSHOT":
  ...
  async handleGenerateUI(message, sender) {
    try {
  ...
  async handleGetScreenshot(message) {
    try {
  ...
  async handleSaveCustomFeature(message) {
    try {
  ...
  async handleGetCustomFeatures(message) {
    try {
  ...
  async handleToggleFeatureAutoApply(message) {
    try {
  ...
  async handleUpdateGlobalSettings(message) {
    try {
  ...
  async handleGetSiteStatus(message) {
    try {
  ...
  async handleToggleSiteDisabled(message) {
    try {
  ...
  async handleInjectScriptViaAPI(message, sender) {
    try {
